...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    if(nHeight < BLOCKCOUNT)
        nRewardCoinYear = 3 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (2 * BLOCKCOUNT))
        nRewardCoinYear = 2.5 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (3 * BLOCKCOUNT))
        nRewardCoinYear = 2 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (4 * BLOCKCOUNT))
        nRewardCoinYear = 1.5 * 4* MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (5 * BLOCKCOUNT))
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (nActualSpacing < 0)
        nActualSpacing = fProofOfStake? nStakeTargetSpacing : nTargetSpacing;
    ...
    int64_t nTargetSpacing_Final = fProofOfStake? nStakeTargetSpacing : nTargetSpacing;
    int64_t nInterval = nTargetTimespan / nTargetSpacing_Final;
    bnNew *= ((nInterval - 1) * nTargetSpacing_Final + nActualSpacing + nActualSpacing);
    bnNew /= ((nInterval + 1) * nTargetSpacing_Final);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, pindexBest->nHeight);
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        bnProofOfStakeLimit = bnProofOfWorkLimitTestNet;
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("65995a37d907ad057c0b06a346176fb46495575477e06921362fee3a934610ac"));
        block.print();
